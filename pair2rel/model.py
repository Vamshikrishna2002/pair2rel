...
import pair2rel
from pair2rel.modules.layers import LstmSeq2SeqEncoder, ScorerLayer, FilteringLayer, RefineLayer
from pair2rel.modules.base import InstructBase
from pair2rel.modules.evaluator import greedy_search, RelEvaluator
from pair2rel.modules.span_rep import SpanRepLayer
from pair2rel.modules.rel_rep import RelRepLayer
from pair2rel.modules.token_rep import TokenRepLayer
from pair2rel.modules import loss_functions
...
class Pair2Rel(InstructBase, PyTorchModelHubMixin):
    ...
    def _from_pretrained(
        ...
        **model_kwargs,
        ...
        # Use "pytorch_model.bin" and "pair2rel_config.json"
        ...
        config_file = Path(model_id) / "pair2rel.json"
        if not config_file.exists():
            config_file = hf_hub_download(
                repo_id=model_id,
                filename="pair2rel_config.json",
                revision=revision,
                cache_dir=cache_dir,
                force_download=force_download,
                proxies=proxies,
        ...
        **push_to_hub_kwargs,
        ...
        if config is not None:
            if isinstance(config, argparse.Namespace):
                config = vars(config)
            (save_directory / "pair2rel_config.json").write_text(json.dumps(config, indent=2))
